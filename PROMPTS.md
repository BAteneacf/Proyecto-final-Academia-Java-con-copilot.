...
Usar Copilot cambió mi forma de trabajar: en vez de escribir cada línea, tuve que aprender a describir con precisión lo que necesitaba y, sobre todo, a no confiar ciegamente en lo que generaba. El caso más claro fue en BatchConfig, donde Copilot inventó un import y un método que no existen (MongoItemWriter y setMongoTemplate); si no lo hubiera revisado línea por línea, el proyecto no habría compilado. También aprendí que el umbral de aprobación (>=70) es un punto donde la IA puede fallar por un detalle mínimo, así que ahora reviso ese tipo de condiciones con más cuidado en cualquier código, generado por IA o no. Prompts más específicos (con contexto, restricciones y validación) dieron resultados notablemente mejores que pedidos vagos, lo cual confirma que "saber pedir" es una habilidad en sí misma. En general, sentí que mi rol pasó de escribir código a dirigir y auditar lo que la IA produce, que es justo la habilidad que se supone debía desarrollar en este taller.